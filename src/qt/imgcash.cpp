...
{
    return QCoreApplication::translate("imgcash-core", psz).toStdString();
}
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. IMGCash Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(imgcash);
    Q_INIT_RESOURCE(imgcash_locale);
    ...
    {
        QMessageBox::critical(0, QObject::tr("ImgCash Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("ImgCash Core"), QObject::tr("Error: %1").arg(e.what()));
        return EXIT_FAILURE;
    }
#ifdef ENABLE_WALLET
    ...
    try
    {
        ...
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("ImgCash Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
